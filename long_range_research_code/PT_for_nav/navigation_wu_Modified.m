function [sr, PL_bin, PL_wei, PL_dis, paths] = navigation_wu_Modified(L, D, max_hops)

% Navigation of connectivity length matrix L guided by nodal distance D
% Modified based on navigation_wu.m (in Brain connectivity toolbox)
% % Navigation
...
%
% paths - NxN cell of nodes comprising navigation paths. Modification: Set the failed navigation path sequence to empty. 
            ...
            if (i ~= j)
                ...
                while (curr_node ~= target)
                    ...
                    if isempty(next_node) || next_node == last_node || pl_bin > max_hops
                        ...
                        pl_wei = Inf;
                        pl_dis = Inf;
                        % Modification: Set the failed navigation path sequence to empty.
                        paths{i,j} = [];
                        break; 
